$("#btn-hash").click(function (event) {
    ...
    reader.onload = (function (theFile) {
        return function (e) {
            ...
            console.log("binary");
            console.log(binary);
            const detachedStamped =
                OpenTimestamps.DetachedTimestampFile.fromBytes(op, binary);
...
$("#btn-stamp").click(function (event) {
    ...
    OpenTimestamps.stamp(detachedOriginal, param)
        .then(() => {
...
$("#btn-load").click(function (event) {
    ...
    reader.onload = (function (theFile) {
        return function (e) {
            ...
            const detachedStamped =
                OpenTimestamps.DetachedTimestampFile.deserialize(ots);
            const digest = detachedStamped.fileDigest();
            const hashValue = bytesToHex(digest);
            ...
            $("#verify-ots").val(hexots);
            $("#verify-output").val("Result will be displayed here");
            const upgradedOTSfilename = filename.replace(
                ".ots",
                ".upgraded.ots"
            );
...
$("#btn-upgrade").click(function (event) {
    ...
    OpenTimestamps.upgrade(detachedStamped, param)
        .then((changed) => {
        ...
        .then((results) => {
            if (Object.keys(results).length === 0) {
                ...
                Object.keys(results).map((chain) => {
                    const date = moment(results[chain].timestamp * 1000)
                        .tz(moment.tz.guess())
                        .format("YYYY-MM-DD z");
                    outputText +=
                        "\n" +
                        upperFirstLetter(chain) +
                        " block " +
                        results[chain].height +
