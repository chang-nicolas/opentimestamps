...
const whitelistedCalendars = new OpenTimestamps.Calendar.UrlWhitelist(
  calendarsList
);
...
function stamp(filename, hash, hashType) {
  ...
  OpenTimestamps.stamp(detached, options)
    .then(() => {
...
function upgrade_verify(ots, hash, hashType, filename) {
  ...
  OpenTimestamps.upgrade(detachedOts, upgradeOptions)
    .then((changed) => {
      ...
      return OpenTimestamps.verifyTimestamp(
        detachedOts.timestamp,
        blockexplorers
      );
    })
    .then((results) => {
      ...
      if (Object.keys(results).length == 0) {
        ...
        if (detachedOts.timestamp.isTimestampComplete()) {
          ...
          detachedOts.timestamp.allAttestations().forEach((attestation) => {
            if (
              attestation instanceof OpenTimestamps.Notary.UnknownAttestation
            ) {
          ...
          proof_warning(
            "The submission receipt is still in pending attestation."
          );
        }
      } else {
        var text = "";
        Object.keys(results).map((chain) => {
          var date = moment(results[chain].timestamp * 1000)
            .tz(moment.tz.guess())
            .format("YYYY-MM-DD z");
  ...
  show: function () {
    if (this.filename) {
      ...
      $(this.tagId + " .filename").html(
        "Drop here the <b>original</b> file to <b>verify</b> it matches the receipt/proof provided above."
      );
    }
    if (this.filesize) {
      $(this.tagId + " .filesize").html(
        " " + humanFileSize(this.filesize, true)
      );
    } else {
      $(this.tagId + " .filesize").html("&nbsp;");
    }
    ...
    if (!Hashes.progress && Hashes.get(hashType)) {
      $(this.tagId + " .hash").html(
        "<b>hash value</b>:<br>(" + hashType + ") " + Hashes.get(hashType)
      );
    } else {
      $(this.tagId + " .hash").html("&nbsp;");
    }
  ...
  callback: function () {
    ...
    if (Proof.exist()) {
      if (Proof.getHash() == Hashes.get("SHA256")) {
        if (Proof.stamped()) {
          verified(
            "The provided file matches the provided attestation proof: same hash values.<br><b>File existence is attested</b> at the above date."
          );
        } else {
          matched(
            "The provided file matches the provided submission receipt: same hash values.<br>Anyway, <b>attestation is still pending</b>."
          );
        }
      } else {
        if (Proof.stamped()) {
          failure(
            "The provided file does NOT match the provided attestation proof: different hash values.<br><b>File existence is NOT attested</b> at the above date."
          );
        } else {
          failure(
            "The provided file does NOT match the provided submission receipt: different hash values."
          );
        }
      }
    } else {
...
var Proof = {
  ...
  show: function (tagId) {
    ...
    if (this.filesize) {
      ...
      $(this.tagId + " .filesize").html(
        " " + humanFileSize(this.data.length, true)
      );
    }
    ...
    if (Proof.data) {
      ...
      $(this.tagId + " .hash").html(
        "Submitted <b>hash value</b>:<br>(" + hashType + ") " + hash
      );
...
function run_stamping() {
  ...
  if (Hashes.get(hashType)) {
    ...
    failure(
      "To <strong>stamp</strong> you need to drop a file in the Data field"
    );
  }
}
...
function run_verification() {
  if (Proof.data) {
    ...
    proof_failure(
      "To <strong>verify</strong> you need to drop a file in the Data field and a <strong>.ots</strong> receipt in the OpenTimestamps proof field"
    );
  }
}
...
function run_info() {
  if (Proof.data) {
    ...
    failure(
      "To <strong>info</strong> you need to drop a file in the Data field and a <strong>.ots</strong> receipt in the OpenTimestamps proof field"
    );
  }
}
